	...
	function getjjrszModelByYear(year) {
	    jjrmodelidlist = ['1', '2'];
	    jjrmodeltimelist = ['2017-07-12 00:00:00', '2017-07-13 00:00:00']; //这里时间的格式为yyyy-MM-dd HH:mm:ss
	    jjrmodelztlist = ['2', '1']; //1为上班，2为休息
	    //	$.ajax({
	    //		type:"POST",
	    //		url:,
	    //		async:false,
	...
	//回到今天
	//
	//左边日历选择的时候  右边也要对应当前选择的日期
	//
	//周六日标红  其余的显示默认颜色   当鼠标点击非周六日时候  右侧的下拉框显示"今天是休息日" 点击的是周六日的 时候  下拉框可以选择下拉选项  点击提交 左边的已选择的日期 标记 下拉框选择的值
	...
	function setRigth(year, yue, day) {
	    ...
	    var $select = $('<select style="width: 129.6px;"></select>');

	    $('.aboluo-ssjjr').append('<span class="select-title"　 style="display:block">今日为工作日</span>');
	    $select.appendTo($('.aboluo-ssjjr'));
	    ...
	    $('.aboluo-ssjjr').append(szrqbutton);

	    var $submitBtn = $(szrqbutton);
	    // var $select = $(".aboluo-ssjjr s");

	    if (week == '星期六' || week == '星期日') {
	        $('.select-title').text('双休日')
	        $select.show();
	        ...
	        $.ajax({
	            ...
	            success: function(users) {
	                for (var i = 0; i < users.length; i++) {
	                    ...
	                    $('<option>' + users[i].name + '</option>').appendTo($select);
	    ...
	    if (bool) {
	        szrqselect.options.add(new Option("111", "1"));
	    } else {
	        szrqselect.options.add(new Option("今日为工作日", "2"));
	    }
